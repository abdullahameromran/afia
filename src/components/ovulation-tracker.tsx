...
const ovulationFormSchema = z.object({
...
type OvulationFormValues = z.infer<typeof ovulationFormSchema>;

const LOCAL_STORAGE_KEY = 'ovulationTrackerData';
...
export function OvulationTracker() {
  ...
  const form = useForm<OvulationFormValues>({
  ...
  useEffect(() => {
    ...
    if (storedData) {
      try {
        const parsedData = JSON.parse(storedData) as Partial<OvulationFormValues & { lastPeriodDate: string }>;
        ...
        const validatedDefaultValues: Partial<OvulationFormValues> = {};
        ...
        if (Object.keys(validatedDefaultValues).length > 0) {
          ...
          if (resetValues.lastPeriodDate) {
            calculateOvulation(resetValues.lastPeriodDate, resetValues.cycleLength);
          }
        }
      } catch (error) {
        console.error("Failed to parse ovulation data from localStorage", error);
  ...
  const onSubmit: SubmitHandler<OvulationFormValues> = (data) => {
    calculateOvulation(data.lastPeriodDate, data.cycleLength);
  ...
  return (
    <div className="space-y-6 text-right">
      ...
      {estimatedOvulationDate && fertileWindowStart && fertileWindowEnd && (
        <Alert variant="default" className="bg-primary/10 border-primary text-primary text-right">
          ...
          <AlertTitle className="font-semibold">فترة التبويض المتوقعة</AlertTitle>
          <AlertDescription>
            <p>يوم التبويض المتوقع: {estimatedOvulationDate}</p>
            <p>نافذة الخصوبة المتوقعة: من {fertileWindowStart} إلى {fertileWindowEnd}</p>
          </AlertDescription>
        </Alert>
      )}
      ...
      <Alert variant="default" className="mt-4 text-right">
        ...
        <AlertDescription>
          هذه الأداة تقدم تقديرات لأغراض إعلامية عامة. قد يختلف توقيت التبويض الفعلي. لا تعتمدي على هذه التقديرات كوسيلة وحيدة لتحديد النسل أو التشخيص. استشيري طبيبكِ دائمًا.
        </AlertDescription>
      </Alert>
    </div>
  );
